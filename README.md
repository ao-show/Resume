...
- 某自動車メーカー様が利用するシステムで、自動車の次期モデル開発に役立てたり、現行車両における改善点の把握を目的としたシステム。
- 検証対象の車両に専用の車載機が搭載されており、その車載機からAWS S3に向けて車両データが送信される。
- AWS S3に蓄積された車両データをもとに、ETL処理を行うバッチ(以下ETLバッチ)によってデータを変換。変換後のデータをWebアプリケーションによって可視化することで、先述した目的を実現する。
...
- 本件で使用する言語やAWS、Linuxなどの全てが未経験の状態でアサインとなったが、1ヶ月で基本的な知識や操作などを習得。以後はETLバッチの開発および改修をほぼ一人で担当しつつ、手が空いた際にはWebアプリケーション側の改修に回るなどし、自分の持つ知識や技術、経験をフルコミットすることでチームに貢献した。
...
- 本件の開発担当は自分のみで周りに技術面で相談出来る相手がいない状況であったが、最後まで責任をもって対応し納品まで完了させた。初めて一人でコーディング～納品まで行ったことで、自分の技術力が通用することを実感し、責任感をもって最後まで対応出来る人間であるということを、上司を含めた社内全員にアピールすることが出来た。
...
- 車両の速度や揺れ、アラートなどの情報がAzure上に送信され、そのデータをもとに、監視対象車両の走行軌跡が描画されたマップであったり運転診断結果などを閲覧出来るWebアプリケーションをBtoCとして提供。
...
- メンバー内にグラフのライブラリについて精通している者がいなく、自分が主体となってライブラリの選定を担当した。各ライブラリの公式ドキュメント(英語)などを読んで、やりたいことが実現出来るかどうかを検討し、チーム内のメンバーに情報を展開した。
...
- テスト仕様書の作成やテスト実施、デバッグなどの与えられた作業をこなしつつ、空いた時間で他人のソースコードを見たりネットで調べたりすることでプログラミングについて学習した。プロジェクト自体が忙しかったこともあってサーバサイドは担当させてもらえなかったが、今自分が出来ることをアピールしたことでフロントエンド側を一部任せてもらうことができ、自らが持つ積極性やコミュニケーション能力の高さを発揮した。
